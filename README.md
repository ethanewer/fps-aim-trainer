...
- Default presets come from `scripts/default-tasks.py`, which also defines size radii, wall distances, movement speeds, and bounce physics. A default task is size, wall (`close` / `mid` / `far`), a fixed target count, movement (`static` / `strafing` / `dynamic` / `bounce`), mode (`clicking` / `tracking`), and health. Clicking defaults are one-shot. Target-switching defaults are tracking copies of the dynamic/strafe clicking tasks (health `20` on dynamic, `10` on strafe). Tracking defaults are one small dynamic target with infinite health. `BOUNCE 180` is a six-target clicking bounce task, and `BOUNCE 180 SWITCHING 40` is a six-target bounce copy with tracking mode and 40 health. Clicking and switching currently use 3 targets on mid, close, and far. Mid wall is omitted from the name; close and far append `CLOSE` or `FAR`; switching names include `SWITCHING` and the health value; tracking names include `TRACKING`. Edit the catalog in that file and click **Reset tasks** to reload it immediately (no rebuild). Generation plumbing is `scripts/default_tasks.py`. `python scripts/default-tasks.py` still regenerates the committed JSON/C++ table for the next compile.
...
- `scripts/default-tasks.py` — default-task catalog.
- `scripts/default_tasks.py` — catalog plumbing (JSON, C++, Reset-tasks dump).
